...
export const addScheduledMessage = (message: Omit<ScheduledMessage, "id" | "createdAt">): ScheduledMessage => {
...
export const updateScheduledMessage = (id: string, updates: Partial<ScheduledMessage>): void => {
  const stored = localStorage.getItem(SCHEDULED_KEY)
  const messages: ScheduledMessage[] = stored ? JSON.parse(stored) : []
...
export const processScheduledMessages = async (): Promise<void> => {
  ...
  const pendingMessages = messages.filter((m) => m.status === "pending" && new Date(m.scheduledFor) <= now)
