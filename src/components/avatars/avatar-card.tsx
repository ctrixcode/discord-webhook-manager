...
export function AvatarCard({ avatar, onEdit, onDelete, onSelect, selectable = false }: AvatarCardProps) {
    ...
    <>
      <Card className="bg-slate-900/50 backdrop-blur-sm border-slate-700/50 hover:bg-slate-800/50 transition-all duration-200">
        <CardContent className="p-4">
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
              <Avatar className="w-12 h-12 ring-2 ring-purple-500/20">
                <AvatarImage src={avatar.avatarUrl || "/placeholder.svg"} alt={avatar.username} />
          ...
          <div className="text-xs text-slate-500 mb-3">Created: {new Date(avatar.createdAt).toLocaleDateString()}</div>

          {selectable && (
            <Button onClick={() => onSelect?.(avatar)} className="w-full bg-purple-600 hover:bg-purple-700 text-white">
              Select Avatar
            </Button>
          )}
        </CardContent>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent className="bg-slate-800 border-slate-700">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-slate-700 text-slate-300 hover:bg-slate-600">Cancel</AlertDialogCancel>
