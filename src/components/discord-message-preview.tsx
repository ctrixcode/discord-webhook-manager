...
function parseDiscordMarkdown(text: string): ReactNode[] {
  ...
  const patterns = [
    ...
    { regex: /\*\*(.*?)\*\*/g, render: (match: string) => <strong>{match}</strong> }, // **bold**
    ...
    { regex: /~~(.*?)~~/g, render: (match: string) => <s>{match}</s> }, // ~~strikethrough~~
    {
      regex: /`(.*?)`/g,
      render: (match: string) => (
        <code className="bg-[#202225] px-1 py-0.5 rounded text-[#f8f8ff] text-xs">{match}</code>
      ),
    }, // `code`
    {
      regex: /\|\|(.*?)\|\|/g,
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2 mb-0.5">
            <span className="font-medium text-[#f2f3f5] text-base hover:underline cursor-pointer">{username}</span>
                ...
                <div className="flex">
                  {embed.color && (
                    <div
                      ...
                      style={{ backgroundColor: discordColorToHex(embed.color) }}
                    />
                  )}
                  <div
                    className="bg-[#2b2d31] rounded-r-md rounded-l-sm p-4 flex-1 border-l-4"
                    style={{
                      borderLeftColor: embed.color ? discordColorToHex(embed.color) : "transparent",
                    }}
                  >
                    {embed.author && (
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-[#f2f3f5]">
                          {embed.author.url ? (
                            <a href={embed.author.url} className="text-[#00aff4] hover:underline">
                              {embed.author.name}
                            </a>
                          ) : (
                            embed.author.name
                    ...
                    {embed.image && (
                      <div className="mb-3">
                        <img src={embed.image.url || "/placeholder.svg"} alt="" className="max-w-full rounded" />
                      </div>
                    )}
