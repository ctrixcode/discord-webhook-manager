...
const PlanFeature: React.FC<PlanFeatureProps> = ({ text, available }) => (
  <div className="flex items-center gap-2 text-sm">
    <CheckCircle2 className={`h-4 w-4 ${available ? 'text-green-500' : 'text-gray-500'}`} />
    <span className={available ? 'text-slate-300' : 'text-slate-500 line-through'}>{text}</span>
  </div>
);
...
export default function PlansPage() {
  const plans = [
    ...
    },
    {
      ...
      cta: 'Upgrade',
      ctaLink: '#',
      badgeClass: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30 shadow-lg shadow-yellow-500/20',
    },
  ];
          ...
          >
            <div className="flex flex-col h-full">
              <div className="mb-4">
                ...
                <Badge className={`mt-2 text-sm px-3 py-1 font-semibold ${plan.badgeClass}`}>
              ...
              <ul className="space-y-2 text-slate-400 flex-grow">
                {plan.features.map((feature, index) => (
                  <PlanFeature key={index} text={feature.text} available={feature.available} />
                ))}
              </ul>
