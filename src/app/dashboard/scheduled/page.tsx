...
import { getScheduledMessages, processScheduledMessages, type ScheduledMessage } from "@/lib/scheduled-storage"
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage your scheduled Discord messages</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={handleProcessMessages} disabled={isProcessing}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-slate-900/20 backdrop-blur-sm border-slate-700/50">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-200">Total Scheduled</CardTitle>
        ...
        <Card className="bg-slate-900/20 backdrop-blur-sm border-slate-700/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">{pendingMessages}</div>
          </CardContent>
        </Card>

        <Card className="bg-slate-900/20 backdrop-blur-sm border-slate-700/50">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-200">Sent</CardTitle>
        ...
        <Card className="bg-slate-900/20 backdrop-blur-sm border-slate-700/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-white">{failedMessages}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          ...
          <SelectContent className="bg-slate-900/95 backdrop-blur-sm border-slate-700/50">
            <SelectItem value="all" className="text-white hover:bg-slate-800/50">
              All statuses
            </SelectItem>
            <SelectItem value="pending" className="text-white hover:bg-slate-800/50">
              Pending
            </SelectItem>
            <SelectItem value="sent" className="text-white hover:bg-slate-800/50">
              Sent
            </SelectItem>
            <SelectItem value="failed" className="text-white hover:bg-slate-800/50">
              Failed
            </SelectItem>
            <SelectItem value="cancelled" className="text-white hover:bg-slate-800/50">
              Cancelled
            </SelectItem>
          </SelectContent>
        </Select>
      ...
      {filteredMessages.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMessages.map((message) => (
            <ScheduledMessageCard key={message.id} message={message} onMessageUpdated={loadMessages} />
          ))}
        </div>
      ) : messages.length === 0 ? (
        <Card className="bg-slate-900/20 backdrop-blur-sm border-slate-700/50">
